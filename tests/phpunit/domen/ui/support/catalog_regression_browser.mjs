...
try {
	...
	if (scenario === 'filter') {
		...
		const catalog = await evaluate(client, `(() => { const cards = Array.from(document.querySelectorAll('.catalog-card')).slice(0, 3); const media = cards.map(card => card.querySelector('.catalog-card__media')?.getBoundingClientRect()).filter(Boolean); const bottoms = cards.map(card => Math.round(card.querySelector('.catalog-card__actions')?.getBoundingClientRect().bottom || 0)); return { equalMedia: media.length === 3 && media.every(rect => Math.round(rect.width) === Math.round(media[0].width) && Math.round(rect.height) === Math.round(media[0].height)), maxMediaHeight: Math.max(...media.map(rect => rect.height)), objectFits: [...new Set(cards.map(card => getComputedStyle(card.querySelector('.catalog-card__image-primary')).objectFit))], actionsAligned: bottoms.length === 3 && bottoms.every(bottom => Math.abs(bottom - bottoms[0]) <= 1) }; })()`);
		await navigate(client, new URL('?route=product/search', targetUrl).href);
