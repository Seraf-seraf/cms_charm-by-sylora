...
final class SitemapFeedContractTest extends TestCase {
	...
	public function testSitemapContainsOnlyIndexableCatalogPages(): void {
		$controller = file_get_contents(
			dirname(__DIR__, 4) . '/upload/catalog/controller/extension/feed/google_sitemap.php'
		);

		self::assertIsString($controller);
		self::assertStringNotContainsString("load->model('catalog/manufacturer')", $controller);
		self::assertStringNotContainsString('<changefreq>', $controller);
		self::assertStringNotContainsString('<priority>', $controller);
		self::assertStringContainsString('hasActiveProducts((int)$result[\'category_id\'])', $controller);
	}
}
