...
final class CatalogSchemaTest extends PHPUnit\Framework\TestCase {
	public function testBuildsCollectionItemsAndBreadcrumbsWithPagePositions(): void {
		...
		self::assertSame('Длинное название украшения', $data['@graph'][1]['itemListElement'][0]['name']);
		self::assertSame('https://example.test/product&id=42', $data['@graph'][1]['itemListElement'][0]['item']);
		self::assertStringNotContainsString('"@type":"Product"', $json);
	...
	public function testListingControllersUseCanonicalProductLinks(): void {
		$root = dirname(__DIR__, 4);

		foreach (array('category', 'search', 'special', 'manufacturer') as $route) {
			$controller = file_get_contents($root . '/upload/catalog/controller/product/' . $route . '.php');
